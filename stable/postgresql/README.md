...
| `images.postgresql.registry`                  | PostgreSQL Image registry (overrides `global.imageRegistry`)                                                                                                              | `nil`                                                         |
| `images.postgresql.name`                      | PostgreSQL Image name                                                                                                                                                     | `bitnami/postgresql`                                          |
| `images.postgresql.tag`                       | PostgreSQL Image tag                                                                                                                                                      | `{TAG_NAME}`                                                  |
| `images.postgresql.pullPolicy`                | PostgreSQL Image pull policy (overrides `global.imagePullPolicy`)                                                                                                         | `nil`                                                         |
| `images.postgresql.pullSecrets`               | PostgreSQL Image pull secrets (concatenated with `global.imagePullSecrets` and `images.*.pullSecrets`)                                                                    | `[]` (does not add image pull secrets to deployed pods)       |
| `images.postgresql.debug`                     | Specify if debug values should be set                                                                                                                                     | `false`                                                       |
| `images.metrics.registry`                     | PostgreSQL metrics Image registry (overrides `global.imageRegistry`)                                                                                                      | `nil`                                                         |
| `images.metrics.name`                         | PostgreSQL metrics Image name                                                                                                                                             | `bitnami/postgresql`                                          |
| `images.metrics.tag`                          | PostgreSQL metrics Image tag                                                                                                                                              | `{TAG_NAME}`                                                  |
| `images.metrics.pullPolicy`                   | PostgreSQL metrics Image pull policy (overrides `global.imagePullPolicy`)                                                                                                 | `nil`                                                         |
| `images.metrics.pullSecrets`                  | PostgreSQL metrics Image pull secrets (concatenated with `global.imagePullSecrets` and `images.*.pullSecrets`)                                                            | `[]` (does not add image pull secrets to deployed pods)       |
| `images.volumePermissions.registry`           | Init container volume-permissions registry (overrides `global.imageRegistry`)                                                                                             | `nil`                                                         |
| `images.volumePermissions.name`               | Init container volume-permissions name                                                                                                                                    | `bitnami/postgresql`                                          |
| `images.volumePermissions.tag`                | Init container volume-permissions tag                                                                                                                                     | `{TAG_NAME}`                                                  |
| `images.volumePermissions.pullPolicy`         | Init container volume-permissions pull policy (overrides `global.imagePullPolicy`)                                                                                        | `nil`                                                         |
| `images.volumePermissions.pullSecrets`        | Init container volume-permissions pull secrets (concatenated with `global.imagePullSecrets` and `images.*.pullSecrets`)                                                   | `[]` (does not add image pull secrets to deployed pods)       |
...
To modify the PostgreSQL version used in this chart you can specify a [valid image tag](https://hub.docker.com/r/bitnami/postgresql/tags/) using the `images.postgresql.tag` parameter. For example, `images.postgresql.tag=12.0.0-debian-9-r0`
...
helm install postgres \
             --set images.postgresql.name=postgres \
             --set images.postgresql.tag=10.6 \
