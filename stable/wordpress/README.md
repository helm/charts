...
This chart provides support for Ingress resource. If you have available
an Ingress Controller such as Nginx or Traefik you maybe want to set up
`ingress.enabled` to true.

### Hosts
Most likely you will likely only want to have one hostname that maps to
this wordpress installation, however it is possible to have more than
one host.  To facilitate this, the `ingress.hosts` object is an array.
...
`tlsSecret` references does not have to be generated by this chart, but
the ingress record will not work until this secret does exist.
...
* helm values generate / manages certificate secrets
* user geenrates / manages certificates separately
