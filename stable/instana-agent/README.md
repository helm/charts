...
| `agent.updates.version`            | Which version of the updates pack to use without further updates (version pinning), specified as the hash of one of the commits of the [Instana Agent Updates](https://github.com/instana/agent-updates/commits/public) repository. This setting is applicable only to the [`instana/agent` image](https://hub.docker.com/r/instana/agent/) and its derivatives. If specified, this setting overrides `agent.updates.dynamic.frequency` and `agent.updates.dynamic.time`.                              | `{}`                                                                                                        |
| `agent.updates.frequency`            | How often should the Instana agent update itself. Valid values are `DAY` (default value, results in daily updates), `MONDAY`, `TUESDAY`, `WEDNESDAY`, `THURSDAY`, `FRIDAY`, `SATURDAY` and `SUNDAY`; values other than `DAY` can be specified together, comma separated. This setting is applicable only to the [`instana/agent` image](https://hub.docker.com/r/instana/agent/) and its derivatives.                             | `{}`                                                                                                        |
| `agent.updates.time`            | Time of day when the Instana agent update itself on days matching `agent.updates.frequency`. The value is expected in `hh:mm` format, UTC timezone. Default is '04:30'. This setting is applicable only to the [`instana/agent` image](https://hub.docker.com/r/instana/agent/) and its derivatives.                              | `{}`                                                                                                        |
