...
| `resources`                        | CPU/Memory resource requests/limits    | Memory: `768Mi`, CPU: `600m`                |
| `persistence.enabled`              | Enable persistence using PVC           | `true`                                      |
| `persistence.apache.storageClass`  | PVC Storage Class for Apache volume    | `nil` (uses alpha storage class annotation) |
| `persistence.apache.accessMode`    | PVC Access Mode for Apache volume      | `ReadWriteOnce`                             |
| `persistence.apache.size`          | PVC Storage Request for Apache volume  | `1Gi`                                       |
| `persistence.apache.existingClaim` | If PVC exists&bounded for Apache       | `nil` (when nil, new one is requested)      |
...
| `persistence.moodle.existingClaim` | If PVC exists&bounded for Moodle       | `nil` (when nil, new one is requested)      |
