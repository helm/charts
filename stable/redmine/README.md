...
| `existingSecret`                    | Use existing secret for password details (`redminePassword`, `smtpPassword` and `externalDatabase.password` will be ignored and picked up from this secret). The secret must contain the keys `redmine-password`, `smtp-password` and `external-db-password`. | `nil`                        |
