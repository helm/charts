...
{{/*
Return the proper image name (for the init container force-load image)
*/}}
{{- define "rabbitmq.forceLoad.image" -}}
{{- $registryName := .Values.forceLoad.image.registry -}}
{{- $repositoryName := .Values.forceLoad.image.repository -}}
{{- $tag := .Values.forceLoad.image.tag | toString -}}
{{/*
Helm 2.11 supports the assignment of a value to a variable defined in a different scope,
but Helm 2.9 and 2.10 doesn't support it, so we need to implement this if-else logic.
Also, we can't use a single if because lazy evaluation is not an option
