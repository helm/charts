...
| `kvprovider.backupAcme.enabled`        | Enable Backup of ACME certificates which are stored in a KV Provider.                                                        | `false`                                           |
| `kvprovider.backupAcme.schedule`       | Crontab Entry for the Backupjob.                                                                                             | `0 0 * * *`                                       |
| `kvprovider.backupAcme.backend`        | Currently only ETCD is supported by the default Docker Image, but you can create your own Image for your KV Provider.        | `etcd`                                            |
| `kvprovider.backupAcme.image`          | Docker Image for the Backup Container.                                                                                       | `crowdfox/traefik-backup-image`                   |
| `kvprovider.backupAcme.imageTag`       | Docker Image Tag for the Backup Container.                                                                                   | `latest`                                          |
