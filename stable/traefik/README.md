...
| `ssl.useNamedCertificateSecret`        | Whether to pull TLS certificate + key from a pre-existing Kubernetes secret                                                  | false                                             |
| `ssl.certificateSecretName`            | Name of Kubernetes secret resource in same namespace as Traefik deployment. Must contain `tls.crt` and `tls.key` objects.    | None                                              |
