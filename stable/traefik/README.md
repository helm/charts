...
| `ssl.defaultCertSecret.enabled`        | Whether to use an existing secret for default certificate/key. If it's `true` the `defaultCert`, the `defaultKey` and `generateTLS` parameters will be ignored.| false           |
| `ssl.defaultCertSecret.name`           | Name of the secret to use for default certificate/key                                                                        | `default-cert`                                              |
| `ssl.defaultCertSecret.secretKey.crt`  | Name of the key in the secret which contain the certificate                                                                  | `tls.crt`                                         |
| `ssl.defaultCertSecret.secretKey.key`  | Name of the key in the secret which contain the certificate key                                                              | `tls.key`                                         |
