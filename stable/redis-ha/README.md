...
| `redis.max_memory`               | Sets the maximum memory redis can allocate            | "" (This defaults to the maximum memory allowed)          |
| `redis.max_memory_policy`        | Sets the max memory policy used by redis              | "" (This defaults to volatile-lru)                        |
| `redis.rdb_backups`              | Determines if scheduled RDB backups will be created   | `false`                                                   |
| `redis.diskless_replication`     | Directly sends the RDB over the wire to slaves, without using the disk as intermediate storage| `false`           |
