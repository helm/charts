...
| `auth.enabled`       | Specifies whether redis use authentication | `false` |
| `auth.secret.name`       | Name of redis existing secret (ignores redisPassword) ||
| `auth.secret.key`       | Key of redis existing secret ||
| `auth.redisPassword`       | Point the redis password when not stored in a secret ||
