...
- Or an [IAM Role for service account](https://aws.amazon.com/blogs/opensource/introducing-fine-grained-iam-roles-service-accounts/) attached to a service account with an annotation. If you change the value for `securityContext.runAsUser` please use same value for `securityContext.fsUser` to be able to access the mounted secret.
