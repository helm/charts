...
`agent.probes.liveness.initialDelaySeconds` | Number of seconds after the container starts before the liveness probes start | `3`
`agent.probes.liveness.periodSeconds` | How often (in seconds) the probe will be performed | `3`
`agent.probes.liveness.timeoutSeconds` | Number of seconds after which the probe will timeout waiting for the command to finish | `1`
`agent.probes.liveness.successThreshold` | Number of successful checks needed to pass before considering the agent live | `1`
`agent.probes.liveness.failureThreshold` | Number of failed checks needed to pass before considering the agent dead | `3`
...
`server.probes.serverAddress` | Address that readiness and liveness probes will hit | `localhost`
`server.probes.serverAddressRefresh` | How often server address will be refreshed | `2s`
`server.probes.timeout` | How long the `/kiam health` command will wait before timing out | `5s`
`server.probes.liveness.initialDelaySeconds` | Number of seconds after the container starts before the liveness probes start | `10`
`server.probes.liveness.periodSeconds` | How often (in seconds) the probe will be performed | `10`
`server.probes.liveness.timeoutSeconds` | Number of seconds after which the probe will timeout waiting for the command to finish | `10`
`server.probes.liveness.successThreshold` | Number of successful checks needed to pass before considering the server live | `1`
`server.probes.liveness.failureThreshold` | Number of failed checks needed to pass before considering the server dead | `3`
`server.probes.readiness.initialDelaySeconds` | Number of seconds after the container starts before the readiness probes start | `10`
`server.probes.readiness.periodSeconds` | How often (in seconds) the probe will be performed | `10`
`server.probes.readiness.timeoutSeconds` | Number of seconds after which the probe will timeout waiting for the command to finish | `10`
`server.probes.readiness.successThreshold` | Number of successful checks needed to pass before considering the server ready | `1`
`server.probes.readiness.failureThreshold` | Number of failed checks needed to pass before considering the server unhealthy | `3`
