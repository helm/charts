...
{{- $altNames := list (include "kiam.server.fullname" .) (printf "%s:%s" (include "kiam.server.fullname" .) .Values.server.service.port) (printf "127.0.0.1:%s" .Values.server.service.targetPort) -}}
{{- $ca := .ca | default (genCA "kiam-ca" 365) -}}
{{- $_ := set . "ca" $ca -}}
