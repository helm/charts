...
| `jmxPort`                      | The jmx port which JMX style metrics are exposed (note: these are not scrapeable by Prometheus)                 | `5555`                                                     |
...
| `prometheus.jmx.configMap.enabled`      | Enable the default ConfigMap for JMX                                                                   | `true`                                                     |
| `prometheus.jmx.configMap.overrideConfig | Allows config file to be generated by passing values to ConfigMap                                     | `{}`                                                       |
| `prometheus.jmx.configMap.overrideName | Allows setting the name of the ConfigMap to be used                                                     | `""`                                                       |
...
| `prometheus.prometheusOperator | True if using the Prometheus Operator, False if not                                                             | `true`                                                       |
| `prometheus.serviceMonitorSelector | Only need for use with the Prometheus Operator, default to kube-prometheus install, but should be set according to Prometheus install    | `{ prometheus: kube-prometheus }` |
